...
//!    `GHOSTWRITER_TEMPERATURE`, `GHOSTWRITER_TIMEOUT_SECS`, so the mcp.json
...
mod tests {
    ...
    #[test]
    fn defaults_resolve() {
        let c = build(None, None, None, None).unwrap();
    ...
    fn build_applies_and_validates_overrides() {
        ...
        assert_eq!(c.timeout, Duration::from_secs(1)); // floored at 1

        assert!(build(Some("ftp://box".into()), None, None, None).is_err());
        assert!(build(None, Some(String::new()), None, None).is_err());
        assert!(build(None, None, Some(f32::NAN), None).is_err());
        assert!(build(None, None, Some(f32::INFINITY), None).is_err());
    }
